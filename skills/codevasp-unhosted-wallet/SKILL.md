...
- `01-Unhosted-Wallet-Introduction.md`: Overview of Unhosted Wallet Verification, integration workflow, required headers, host URLs, webhook setup, and supported networks.
- `02-Issue-Token.md`: API for issuing a one-time verification token and walletVerificationId.
- `03-Render-Widget.md`: Widget script loading, rendering instructions (Vanilla JS & React examples), and client event handling (success/error).
- `04-Get-Result.md`: API for retrieving verification results using the Verification Session ID.
...
1. Always start by scanning `references/api/01-Unhosted-Wallet-Introduction.md` to find authoritative answers about the verification concept, prerequisites, supported networks, and required headers.
...
3. If the user asks about supported networks, refer to the network list in `01-Unhosted-Wallet-Introduction.md` (ETH, ARBITRUM, BASE, KAIA, MATIC, SOL, BSC).
...
1. **Prerequisites & Setup** (Ref: `01-Unhosted-Wallet-Introduction.md`):
...
2. **Step 1 — Issue Token** (Ref: `02-Issue-Token.md`):
...
3. **Step 2 — Render Widget** (Ref: `03-Render-Widget.md`):
   ...
   - Identify the user's preferred framework and provide the corresponding example (Vanilla JS or React) from `03-Render-Widget.md`.

4. **Step 3 — Handle Client Events** (Ref: `03-Render-Widget.md`):
...
5. **Step 4 — Get Verification Result** (Ref: `04-Get-Result.md`):
...
1. **Field Check**: Verify all required fields are present (`blockchain`, `asset`, `address`, `customerIdentification`, `widgetRenderingOrigin`) against `02-Issue-Token.md`.
2. **Network Check**: Confirm the `blockchain` value matches one of the supported networks listed in `01-Unhosted-Wallet-Introduction.md`.
