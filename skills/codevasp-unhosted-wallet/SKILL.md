...
description: Expert guidance on CodeVASP Unhosted_Wallet Verification API integration. Use for wallet ownership verification via signature proof, widget rendering, and verification result retrieval.
metadata: 
...
- **2_Development/**
  - `Dev 01 - Dev Environment Setup.md`: Dashboard onboarding, dev server registration (key pair generation), and IP whitelisting procedures.
  - `Dev 03 - Header Parameter.md`: Detailed specification of mandatory HTTP headers (`X-Code-Req-Nonce`, `X-Code-Req-PubKey`, `X-Code-Req-Signature`, `X-Code-Req-Datetime`, `X-Request-Origin`).

### 2. API Reference (`references/api/`)

- **Unhosted_Wallet/**
  - `06-1. Intro.md`: Overview of Unhosted Wallet Verification, integration workflow, required headers, host URLs, webhook setup, and supported networks.
  - `06-2. Issue Token.md`: API for issuing a one-time verification token and walletVerificationId.
  - `06-3. Render Widget.md`: Widget script loading, rendering instructions (Vanilla JS & React examples), and client event handling (success/error).
  - `06-4. Get Result.md`: API for retrieving verification results using the Verification Session ID.
...
1. Always start by scanning `references/api/Unhosted_Wallet/06-1. Intro.md` and the guides in `references/guides/2_Development/` to find authoritative answers about the verification concept, prerequisites, supported networks, and required headers.
...
1. **Prerequisites & Setup** (Ref: `06-1. Intro.md`, `Dev 01 - Dev Environment Setup.md`, `Dev 03 - Header Parameter.md`):
   - Ensure the Unhosted Wallet feature is activated by contacting the CodeVASP team.
   - Follow the dashboard onboarding and dev server registration steps in `Dev 01 - Dev Environment Setup.md` (account creation, server URL registration, key pair generation).
   - Configure mandatory headers per `Dev 03 - Header Parameter.md`: `X-Code-Req-PubKey`, `X-Code-Req-Signature`, `X-Code-Req-Datetime`, `X-Code-Req-Nonce`, `X-Request-Origin`.
...
2. **Step 1 — Issue Token** (Ref: `06-2. Issue Token.md`):
...
3. **Step 2 — Render Widget** (Ref: `06-3. Render Widget.md`):
   ...
   - Identify the user's preferred framework and provide the corresponding example (Vanilla JS or React) from `06-3. Render Widget.md`.

4. **Step 3 — Handle Client Events** (Ref: `06-3. Render Widget.md`):
...
5. **Step 4 — Get Verification Result** (Ref: `06-4. Get Result.md`):
...
1. **Field Check**: Verify all required fields are present (`blockchain`, `asset`, `address`, `customerIdentification`, `widgetRenderingOrigin`) against `06-2. Issue Token.md`.
2. **Network Check**: Confirm the `blockchain` value matches one of the supported networks listed in `06-1. Intro.md`.
