...
- **API 05 - More Services/**
  - `05-1. Uppsala Wallet Screening.md`: API for screening a wallet address for risk. Returns `securityCategory` and `securityTags`.
  - `05-2. Uppsala TXID Screening.md`: API for screening a blockchain transaction ID for risk. Returns sender/receiver wallet risk info with `securityCategory`, `securityTags`, and `detailedDescription`.
...
1. Always start by scanning `references/api/API 05 - More Services/05-1. Uppsala Wallet Screening.md` and `05-2. Uppsala TXID Screening.md`, as well as the guides in `references/guides/2_Development/`, to find authoritative answers.
...
1. **Prerequisites & Setup** (Ref: `Dev 01 - Dev Environment Setup.md`, `Dev 03 - Header Parameter.md`):
...
2. **Wallet Screening** (Ref: `05-1. Uppsala Wallet Screening.md`):
...
3. **TXID Screening** (Ref: `05-2. Uppsala TXID Screening.md`):
...
1. **Field Check**: Verify all required fields are present:
   - Wallet Screening: `walletAddress`, `chain` (per `05-1. Uppsala Wallet Screening.md`).
   - TXID Screening: `txid`, `chain` (per `05-2. Uppsala TXID Screening.md`).
